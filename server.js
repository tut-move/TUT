...
const server=http.createServer(async(req,res)=>{setSecurityHeaders(res);
 ...
 try{
  ...
  if(/^\/api\/bookings\/[^/]+\/test-pay$/.test(p)&&req.method==='POST'){
    const u=auth(req);if(!u)return json(res,401,{error:'Login required.'});const bid=p.split('/')[3],db=readDB(),b=db.bookings.find(x=>x.id===bid);if(!b)return json(res,404,{error:'Booking not found.'});if(b.buyerUserId!==u.id&&!isOwner(u))return json(res,403,{error:'Only the buyer can run the test payment.'});const dealType=bookingDealType(b,db);if(!b.trip?.ready)return json(res,409,{error:'Agreement checks must be complete before payment.'});b.paymentMode='test';b.paymentStatus='test_authorized';b.testPaymentAt=new Date().toISOString();b.status='payment_tested';addNotification(db,b.providerUserId,'payment','Test payment authorized',`The requester authorized the simulated payment for this ${dealType} agreement.`,'offers');await writeDB(db);return json(res,200,{booking:b,message:'TEST MODE ONLY — no real money was charged.'});
  }
