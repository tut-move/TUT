...
const server=http.createServer(async(req,res)=>{
 ...
 try{
  ...
  if(p==='/api/admin/settings'&&req.method==='PUT'){const u=auth(req);if(!isOwner(u))return json(res,403,{error:'Owner access required.'});const b=await getBody(req),db=readDB();if(Number.isFinite(Number(b.platformFeePct))){const nextFeePct=Math.max(0,Math.min(30,Number(b.platformFeePct)));db.settings.platformFeePct=nextFeePct;for(const booking of db.bookings||[]){if(booking.paymentStatus==='test_authorized')continue;booking.platformFeePct=nextFeePct;booking.platformFee=+(Number(booking.agreedPrice||0)*nextFeePct/100).toFixed(2);booking.buyerTotal=+(Number(booking.agreedPrice||0)+booking.platformFee).toFixed(2);booking.providerNet=Number(booking.agreedPrice||0);booking.feeChargedTo='buyer';}}if(b.defaultCurrency)db.settings.defaultCurrency=String(b.defaultCurrency).slice(0,5);for(const k of ['brandName','siteUrl','ownerName','ownerEmail','legalEntity','supportEmail'])if(k in b)db.settings[k]=String(b[k]||'').trim().slice(0,180);await writeDB(db);return json(res,200,{settings:db.settings});}
  ...
  if(/^\/api\/bookings\/[^/]+\/test-pay$/.test(p)&&req.method==='POST'){
    const u=auth(req);if(!u)return json(res,401,{error:'Login required.'});const bid=p.split('/')[3],db=readDB(),b=db.bookings.find(x=>x.id===bid);if(!b)return json(res,404,{error:'Booking not found.'});if(b.buyerUserId!==u.id&&!isOwner(u))return json(res,403,{error:'Only the buyer can run the test payment.'});const dealType=bookingDealType(b,db);if((dealType==='truck'||dealType==='equipment')&&!b.trip?.ready)return json(res,409,{error:'Vehicle must be ready before payment.'});b.paymentMode='test';b.paymentStatus='test_authorized';b.testPaymentAt=new Date().toISOString();b.status='payment_tested';addNotification(db,b.providerUserId,'payment','Test payment authorized',`The requester authorized the simulated payment for this ${dealType} agreement.`,'offers');await writeDB(db);return json(res,200,{booking:b,message:'TEST MODE ONLY — no real money was charged.'});
  }
  if(/^\/api\/bookings\/[^/]+\/trip-check$/.test(p)&&req.method==='POST'){
    ...
    if(dealType==='transport'&&(b.trip.pickupConfirmed||['in_transit','completed_test'].includes(b.status)))return json(res,409,{error:'Pre-trip verification is locked after pickup.'});
